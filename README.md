...
The `manifest.json` declares the plugin's identity, config schema, supported platforms, and optional setup command. Commands and rules are defined in separate stage files (e.g., `stage-publish.json`). Swift plugins generate these files automatically via the SDK's DSL during `./piqley-build.sh`.

For the full JSON schema of manifest and stage files, see the [JSON Plugin Reference](docs/json-plugin-reference.md).
...
**JSON protocol** (default): piqley sends a JSON request on stdin and the plugin writes JSON lines to stdout. For the full request/response schema, see the [JSON Plugin Reference](docs/json-plugin-reference.md#communication-protocol).
...
Plugins can target multiple platforms. The `piqley-build-manifest.json` declares platform-keyed `bin` and `data` fields that map each platform to its build artifacts. Supported platforms: `macos-arm64`, `linux-amd64`, `linux-arm64`. At least one platform must be declared. When packaged, each platform's files go into subdirectories (`bin/macos-arm64/`, `bin/linux-amd64/`, etc.). When a user installs the plugin, piqley copies only the files matching their platform.

For the full build manifest schema, see the [JSON Plugin Reference](docs/json-plugin-reference.md#piqley-build-manifestjson).
