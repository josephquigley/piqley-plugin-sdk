...
- Packager, Request, ExecutionLog, and UploadCache accept FileSystemManager parameter
- SDK builders and tests use injectable FileSystemManager
...
- Packager and BuildManifest now use `fields.json` instead of `consumed-fields.json`
...
- piqley-core dependency bumped to 0.13.0 (PluginType support)
...
- Minimum PiqleyCore dependency bumped to 0.9.0 (ConfigMetadata)
...
- `BuildManifest.toPluginManifest()` and `buildManifest` builder now set `type: .static` on generated manifests
- `type` field added to `manifest.schema.json` as required enum (`static`, `mutable`)
